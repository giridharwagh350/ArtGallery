...
$eid=$_GET['eid'];
    $query1=pg_query($con,"insert into  tblenquiry(artpdid,fullname,email,mobilenumber,message,enquirynumber) values('$eid','$fullname','$email','$mobilenumber','$message','$enquirynumber')");
        if ($query1) {
 echo '<script>alert("Your enquiry successfully send. Your Enquiry number is "+"'.$enquirynumber.'")</script>';
echo "<script>window.location.href='userin.php'</script>";
  }
  else
    {
